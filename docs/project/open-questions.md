...
## Track C v0.3 — multi-NACE analysis automation (2026-04-29)

Cross-cutting questions surfaced during the Track C build (D-028..D-032). All five are non-blocking for v0.3 PoC sign-off; tracked here so they don't get forgotten in v0.4 planning.

| Date raised | ID | Question | Raised by | Blocking | Status |
|---|---|---|---|---|---|
| 2026-04-29 | OQ-077 | **Per-NACE rule library for Insights + Actions agents.** The 8 langchain.agent system prompts in `n8n-workflows/analyze-publication.json` carry a sector identity + topic anchor list + a generic "lenient relevance / strict tangential mention" rule. Output quality plateaus on edge-case publications (over-tightened prompts produced "all 4 NACEs irrelevant" for a bakery publication; over-loose produced cross-NACE bleed). PM + DE need to design a rule library per NACE — source documents (Czech sector reports, ČSÚ statistics, ČS analyst notes), insight-style guide (Czech-aware, owner-relative when client data available, ~3 insights), per-NACE specifics (e.g., NACE 31 furniture's downstream-consumer-demand exposure vs. NACE 49 freight's fuel + driver-cost sensitivity). | engineer → product-manager, data-engineer | Output quality at scale; v0.4 customer-real-data work. | open — defer to v0.4; user to schedule rule-design session |
| 2026-04-29 | OQ-078 | **Multi-NACE brief edit UX.** Under Model B ([D-029](decision-log.md)) a brief tags multiple NACEs with content stored in `BriefContent.per_nace_content[naceDivision]`. The current admin edit page (`/admin/briefs/[id]/edit`) renders only `content.observations` and `content.closing_actions` (the v0.1/v0.2 top-level shape, which the from-n8n route backfills with the first NACE's content for compat). Per-NACE tabs + side-by-side editing of all NACEs the brief covers is deferred. | engineer, designer | Analyst edit ergonomics for briefs that pass relevance for 2+ NACEs. Today the analyst publishes from the first NACE's content only; non-primary NACE content is in the DB but not editable through the UI. | open — defer to v0.4 |
| 2026-04-29 | OQ-079 | **Brief title format for multi-NACE briefs.** v0.1/v0.2 single-NACE briefs carried the NACE in their title (e.g., "Sektorová analýza — Výroba nábytku — Březen 2026"). Multi-NACE briefs from n8n use the NACE-agnostic format "Sektorová analýza — Duben 2026". PM to confirm desired format: keep agnostic (cleaner across NACEs), prefix with covered NACEs (longer titles, hard if 4 NACEs are all relevant), or attach NACE only when single-NACE-tagged. | product-manager | Brief list readability + email subject lines (when email distribution lands). | open — PM input needed |
| 2026-04-29 | OQ-080 | **Region coverage gap from MagnusWeb Excel.** The new cohort source `Data MagnusWeb.xlsx` ([D-031](decision-log.md)) has no `Obec sídla` / `Město` / city column. Every ingested row has `cohort_companies.cz_region = NULL`. The four-rung degradation ladder in `cohort-compute.ts` falls cleanly to rung 1 (NACE × size_band), but rung 0 (NACE × size × region) is unreachable until the source provides a city column. Acceptable for the PoC — the regional cut was an enhancement rung anyway — but future ingest gates should require a city column. | data-engineer | Future precision of cohort cells. Doesn't block v0.3 (rung-1 percentiles are computed correctly and clearly footnoted). | open — defer to next ingest |
| 2026-04-29 | OQ-081 | **NACE 46 + NACE 24 mix in the wholesale group.** The MagnusWeb Excel labels its third group "Velkoobchod s rudami & Výroba hliníku", which mixes NACE 46.72 (wholesale of metal ores) with NACE 24.42 (production of aluminium). The ingest treats the whole group as NACE division 46 per `NACE_LABEL_TO_CLASS` mapping `velkoobchod s rudami & výroba hliníku → 4672`. Some firms in the cohort are aluminium *producers* and don't strictly belong in a wholesale-of-metal cohort. For the PoC the noise is accepted. | data-engineer | Cohort precision for NACE 46 dashboards. Practical impact small at the firm-count we have (~80 rows). | open — defer; future Excel should provide 4-digit NACE per row for clean filtering |
...
- 2026-04-29 — Track C v0.3 build gate (D-028..D-032 landed). OQ-077..081 added (1 EN→PM/DE rule library, 1 EN+PD edit UX, 1 PM title format, 2 DE source-data gaps). All non-blocking for PoC sign-off; deferred to v0.4 or "next ingest".
